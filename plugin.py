...
def doAssign():
    ...
    # xbmc.executebuiltin("XBMC.RunPlugin(plugin://plugin.video.da_inc/library/movie/play/%s)" % imdbnumber)
...
def doPlay():
    ...
    url = "plugin://plugin.video.da_inc/context/%s/%s/play" % (mediatype, dbid)
    log.info("Starting Da inc with: %s" % url)
...
def doDownload():
    ...
    url = "plugin://plugin.video.da_inc/context/%s/%s/download" % (mediatype, dbid)
    log.info("Starting Da inc with: %s" % url)
    xbmc.Player().play(url)
...
def getDbId():
    ...
    elif 'Da inc' in sys.listitem.getfilename():
